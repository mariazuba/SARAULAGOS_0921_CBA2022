...
//========================================================================
 ...
 init_int nedades
 init_int ntallas
 init_matrix mdatos(1,ntime,1,12)
...
//========================================================================
 ...
 // Fpbr=F(ntime)*pbr(i);// este usa multiplicador pbr
  ...
  Zpbr=Fpbr+M;
  CTPp=elem_prod(elem_div(Fpbr,Zpbr),elem_prod(1.-exp(-1.*Zpbr),Npact));
 ...
 Np=(elem_prod(Np,Sp))*T+pre*(alfa_sr*Bp(i,j-minedad)/(beta_sr+Bp(i,j-minedad)));} //
  ...
  Rpp(i,j)=mfexp(log_Rmed); //proyecta R con R_med
  //  Rpp(i,j)=Reclutas(17); //proyecta R con bajo o alto
...
// ESTIMA nm y CV
 ...
 report << Np << endl;
 report << "CTPpt" << endl;  //Captura proyectadas a?o en curso
 report << CTPpt << endl;
