...
x2 <-c(yrs[1]-1,yrs[nyrs]+1) #xlim
  ...
  plot(tallas,rep0$Prob_talla[1,],type="n",las=1, ylim=c(0, 1.2),xlim=c(4,21),
       cex.lab=0.7,cex.axis=0.7,cex.main=0.8,
       ylab="Probabilidad",xlab="Tallas (cm)",
       main="Clave talla-edad",
       xaxp=c(3,30,30/2), xaxs= "i",yaxs= "i")
  for(i in 1:nage){lines(tallas,rep0$Prob_talla[i,]/max(rep0$Prob_talla[i,]),col=i)}
  text(round(rep0$mu_edad,1),1.1,round(rep0$mu_edad,0),cex=0.7)
```
...
```{r Fig31EstatusProyectado, warning=F, include=T, message=F,eval=T, echo=F,fig.height=4,fig.width=4,fig.align="center",fig.path="Figuras/",dev=fig}

source(paste(getwd(),"/funciones/Fn_DiagramaFase.R",sep="")) 



escR<-c('R_med','R_alto','R_bajo')


for(i in 1:length(escR)){

dira<-paste(dir.0,"/cba_septiembre2021",sep="")
setwd(dira)
carpeta<-"/cba_septiembre2021/"
stds0p<- read.table(paste(dir.0,carpeta,'MAT0921s',i,'.std', sep=''),header=T,sep="",na="NA",fill=T)

name0p<-paste("Estatus proyectado 2022",escR[i],sep=' ')

years0p<-c(rep0$YRS,2022)
nyears0p<-length(years0p)
SSBt0p         <- c(subset(stds0p,name=="BD")$value,subset(stds0p,name=="SSBp")$value[3])
SSBt0stdp      <- c(subset(stds0p,name=="BD")$std,subset(stds0p,name=="SSBp")$std[3])
Ft0p           <- c(subset(stds0p,name=="log_F")$value,subset(stds0p,name=="log_Fref")$value[3])
Ft0stdp        <- c(subset(stds0p,name=="log_F")$std,subset(stds0p,name=="log_Fref")$std[3])
BDo0    <- rep0$Bo
BRMS0   <- BDo0*0.55
FRMS0   <- exp(subset(stds0p,name=="log_Fref")$value[3])

DiagramaFase(name0p,FRMS0,BRMS0,SSBt0p,SSBt0stdp,Ft0p,Ft0stdp,years0p,"red2")
#cruz del año previo
lastB0    <- SSBt0p[nyears0p-1]/BRMS0
lastB    <- SSBt0p[nyears0p-1]
lastF    <- exp(Ft0p[nyears0p-1])/FRMS0
# Calculate confidence intervals
Qmult    <- -qnorm((1-(80/100))/2.0)
sbSE     <- SSBt0stdp[nyears0p-1]
sb95     <- c(lastB-Qmult*sbSE,lastB+Qmult*sbSE)
B95      <- sb95/BRMS0
FvSE     <- Ft0stdp[nyears0p-1]
F95      <- c(lastF*exp(-Qmult*FvSE),lastF*exp(Qmult*FvSE))
...
points(lastB0,lastF,pch=19,col=4)
text(lastB0,lastF-0.1,years0p[nyears0p-1],cex=0.8) 
text(2.5,3.5,escR[i])
}
...
tCBA1d<-cbind(percentil=c(seq(10,50,10)),
             CBA_Rmed=round(CBAs1a*0.977,0),
             CBA_Ralto=round(CBAs2a*0.977,0),
             CBA_Rbajo=round(CBAs3a*0.977,0))
kable((tCBA1d))
...
text(20000,0.00021,'b)')
box()

#===============================================================================================================
# plot percentiles de captura
plot(seq(10,50,10),CBAs1a,type="o", ylab="CBA 2022 (toneladas)",xlab="Percentil de Captura (P*)",main="",ylim=c(4000,13500),cex.axis=1,cex.main=1,cex.lab=1,col="blue2")
...
```{r Fig31EstatusProyCapturasupuesta, warning=F, include=T, message=F,eval=T, echo=F,fig.height=4,fig.width=4,fig.align="center",fig.path="Figuras/",dev=fig}

source(paste(getwd(),"/funciones/Fn_DiagramaFase.R",sep="")) 



escR<-c('S_0','S_1','S_2','S_3')
...
```{r Fig31EstatusProyTallassupuesta, warning=F, include=T, message=F,eval=T, echo=F,fig.height=4,fig.width=4,fig.align="center",fig.path="Figuras/",dev=fig}
